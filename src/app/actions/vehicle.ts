...
export async function getCustomerDetails(id: string) {
    // Fetch all customer details in parallel to eliminate sequential bottlenecks
    const [customerRes, vehiclesRes, jobsRes, libraryRes] = await Promise.all([
        db.query('SELECT * FROM customers WHERE id = $1', [id]),
        db.query('SELECT * FROM vehicles WHERE customer_id = $1 ORDER BY created_at DESC', [id]),
        db.query(`
